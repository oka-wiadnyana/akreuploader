...
class Pengaturan extends BaseController
{
    ...
    public function daftar_ip()
    {
        $data_ip = db_connect()->table('target_ip')->get()->getResultArray();
        // dd($data_ip);
        return view('pengaturan/daftar_ip', ['data' => $data_ip]);
    }

    public function modal_ip()
    {
        $id = $this->request->getVar('id');
        $data_ip = db_connect()->table('target_ip')->where('id', $id)->get()->getRowArray();

        return $this->response->setJSON([view('pengaturan/modal_ip', ['data_ip' => $data_ip])]);
    }

    public function insert_ip($ubah = null)
    {
        ...
        if (!$this->validate([
            'ip_amora' => [
                'rules' => 'required',
                'errors' => [
                    'required' => 'IP Amora harus diisi'
                ]
            ],
            'ip_preview' => [
                'rules' => 'required',
                'errors' => [
                    'required' => 'API Preview harus  diisi',
            ...
            session()->setFlashdata('validasi', $this->validation->getErrors());
            return redirect()->to('pengaturan/daftar_ip');
        }


        $id = $this->request->getVar('id');
        $ip_amora = $this->request->getVar('ip_amora');
        $ip_preview = $this->request->getVar('ip_preview');
        ...
        if ($ubah == 'ubah') {
            ...
            if (db_connect()->table('target_ip')->where('id', $id)->update($data_insert)) {
                session()->setFlashdata('success', 'Data berhasil diubah');
                return redirect()->to('pengaturan/daftar_ip');
            } else {
                session()->setFlashdata('validasi', ['Data gagal diubah']);
            ...
            if (db_connect()->table('target_ip')->insert($data_insert)) {
                session()->setFlashdata('success', 'Data berhasil dimasukkan');
                return redirect()->to('pengaturan/daftar_ip');
            } else {
                session()->setFlashdata('validasi', ['Data gagal dimasukkan']);
    ...
    {
        $id = $this->request->getVar('id');
        if (db_connect()->table('target_ip')->where('id', $id)->delete()) {
            session()->setFlashdata('success', 'Data berhasil dihapus');
            return $this->response->setJSON(['msg' => 'success']);
        } else {
            session()->setFlashdata('validasi', ['Data gagal dihapus']);
